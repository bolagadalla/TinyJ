...
public final class CodeInterpreter {
  ...
  // This is the pointer to the data memory location at the offset 0 in the method that is currently being executing
  static int FP = TJ.DATA_MEMORY_SIZE + POINTERTAG;
  
  // The first unallocated data memory location which is above the currently allocated location
  // Used to place data in the data memory location and the followed by increment, and decremented when we deallocate data memory location
  static int ASP;   // initialized within InterpretCode()
  
  // Contains the code memory address of the next instruction to be fetched for execution
  static int PC = 0;
  // The current instruction to be executed
  static Instruction IR;
  // Its a counter for the number of items currently on the expression evaluation stack
  static int ESP = 0;
  ...
  /// This is the fetch-execute loop occurs.
  /// It will fetch an instruction from the `generatedCode` ArrayList and execute it.
  ...
  public static void dump()
  {
      ...
      for (int i = 0; i <= heapTop; i++) {
          ...
          if (d >= POINTERTAG)
            TJ.output.getOutFileWriter().println(" = PTR TO " + (d - POINTERTAG));
      ...
      if (FP == TJ.DATA_MEMORY_SIZE + POINTERTAG)
        ...
        TJ.output.getOutFileWriter().print(" PTR TO " + (FP - POINTERTAG));

      TJ.output.getOutFileWriter().println("  ASP= PTR TO " + (ASP - POINTERTAG)
                             + "\n  HP= PTR TO " + (HP - POINTERTAG)
                             + "  HMAX= PTR TO " + (HMAX - POINTERTAG));
      ...
      for (int i = ESP - 1; i >= 0; i--) {
        int d = EXPRSTACK[i];
        TJ.output.getOutFileWriter().print("  EXPRSTACK[" + i + "]: " + d);
        if (d >= POINTERTAG)
            TJ.output.getOutFileWriter().println(" = PTR TO " + (d - POINTERTAG));
        else
            TJ.output.getOutFileWriter().println();
