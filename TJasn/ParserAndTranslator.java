...
public final class ParserAndTranslator {
  ...
  private static void argumentList(MethodRec m) throws SourceFileErrorException
  {
     ...
     if (m.getArgCount() == -2) {
		m.setArgCount(numbArgs);
     }
     else if(m.getArgCount() != numbArgs) {
    	 throw new SourceFileErrorException("Method " + m.name + " was declared with " + m.getArgCount() + " arguments.");
     }
     accept(RPAREN);
     TJ.output.decTreeDepth();
  }
  ...
  private static void whileStmt() throws SourceFileErrorException
  {
    ...
    /* Complete */
    int nextAddress = Instruction.getNextCodeAddress();
    accept(WHILE);
    accept(LPAREN);
    expr7();
    accept(RPAREN);
  ...
  private static void expr4() throws SourceFileErrorException
  {
    ...
    if (op == GT || op == LT || op == GE || op == LE) {
		...
		case LE:
			new LEinstr();
		default:
			throw new SourceFileErrorException("Unexpected Comparison Operators: " + op);
		}
    }
